...
class Enemy {
  ...
  updateMesh() {
    ...
    // Spawn animation (3x base scale)
    const bs = this._baseScale || 3;
    if (this.spawnTimer > 0) {
      const t = 1 - this.spawnTimer / 0.4;
      this.mesh.scale.setScalar(t * bs);
      return;
    } else {
      // Breathing
      const t = Date.now() / 1000;
      const breath = 1 + Math.sin(t * 2.2 + this.phaseTimer) * 0.08;
      this.mesh.scale.set(bs, breath * bs, bs);
    }

    // Dying animation
    if (this.dying) {
      const t = 1 - this.dyingTimer / 0.3;
      this.mesh.scale.setScalar((1 + t * 0.5) * bs);
